...
const Book = () => {
    ...
    useEffect(() => {
        fetch(`https://protected-basin-55412.herokuapp.com/service/${id}`)
            .then(res => res.json())
                    ...
                    <div style={{ display: serviceData ? 'none' : 'block' }} className="col-md-6">
                        <Form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <Form.Group>
                                ...
                                <Form.Control name="name" defaultValue={loggedInUser.name} type="text" placeholder="Enter Name" ref={register({ required: true })} />
                                {errors.name && <p>This field is required</p>}
                            </Form.Group>
                            <Form.Group>
                                <Form.Label>Email address</Form.Label>
                                <Form.Control name="email" defaultValue={loggedInUser.email} type="email" placeholder="Enter Email" ref={register({ required: true })} />
                                {errors.email && <p>This field is required</p>}
                            </Form.Group>
                            <Form.Group>
                                <Form.Label>Client Number</Form.Label>
                                <Form.Control name="number" type="number" placeholder="Enter Number" ref={register({ required: true })} />
